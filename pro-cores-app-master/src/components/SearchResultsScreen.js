  ...
  return (
    <View style={styles.container}>
      ...
      {data.length > 0 ? <>
        ...
        horizontal
        bounces={false}
        contentContainerStyle={{width: maxTotalWidth, backgroundColor: 'gold'}}
        >
          
          <View style={{ width: maxTotalWidth }} style={{width: maxTotalWidth}}>
            <TouchableOpacity style={{ width: maxTotalWidth }} style={styles.cardTitle} >
              <View style={styles.textView}>
              { (converterState == 2) &&
                <Text style={styles.cardTextTitle} style={{ width: maxWidth, fontSize: 18}} onPress={onPressConverter}>Converter</Text>
              }  
              { (converterState == 1) &&
                <Text style={styles.cardTextTitle} style={{ width: maxWidth, fontSize: 18 }} onPress={onPressConverter}><AntDesign name="caretdown" size={10} color="black"/>Converter</Text>
              }
              { (converterState == 0) &&
                <Text style={styles.cardTextTitle} style={{ width: maxWidth, fontSize: 18 }} onPress={onPressConverter}><AntDesign name="caretup" size={10} color="black"/>Converter</Text>
              }
              { (currentState == 2) &&
                <Text style={styles.cardPriceTitle} onPress={onPressCurrentPrice}>Current Price</Text>
              }
              { (currentState == 1) &&
                <Text style={styles.cardPriceTitle} onPress={onPressCurrentPrice}><AntDesign name="caretdown" size={10} color="black"/>Current Price</Text>
              }
              { (currentState == 0) &&
                <Text style={styles.cardPriceTitle} onPress={onPressCurrentPrice}><AntDesign name="caretup" size={10} color="black" />Current Price</Text>
              }
              { (listState == 2) &&
                <Text style={styles.cardPriceTitle} onPress={onPressListPrice}>List Price</Text>
              }
              { (listState == 1) &&
                <Text style={styles.cardPriceTitle} onPress={onPressListPrice}><AntDesign name="caretdown" size={10} color="black"/>List Price</Text>
              }
              { (listState == 0) &&
                <Text style={styles.cardPriceTitle} onPress={onPressListPrice}><AntDesign name="caretup" size={10} color="black" />List Price</Text>
              }
              </View>
            </TouchableOpacity>
            <ScrollView contentContainerStyle={styles.resultsContainer}>
              {
                ...
                data.map((u, i) => {
                  return (    
                    <TouchableOpacity key={i} style={{width: maxTotalWidth}} style={styles.card} >
                      <View style={styles.textView}>
                        <Text style={styles.cardText} style={{ width: maxWidth, fontSize: 16}}>{u.identifier}</Text>
